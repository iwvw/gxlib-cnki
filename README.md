...
- ⚠️ **防滥用红线（重要）**：平台有反滥用检测，异常高频操作（反复登录/接管、短时间内大量检索/下载）会把账号标记为「涉嫌恶意下载」并**停用**（需联系图书馆申诉）。务必遵守：
  - 不要循环/高频跑 `browser-trust` 或 `login`（单会话策略，每次登录都会踢旧会话，反复登录 = 明显的机器人行为）；
  - 检索/下载保持合理节奏（人工使用频率，检索间隔数秒以上，不要脚本化轰炸）；
  - 若提示「涉嫌恶意下载已被停用」，**立即停止所有自动操作**，联系广西图书馆读者服务申诉，不要再尝试登录（剩余次数有限）。
...
- 下载的 PDF 为版权文献，仅限本人论文写作使用，勿公开传播；`fulltext/` 已 gitignore。
